...
# Add Rolling 7-Day Moving Averages of Tested, Confirmed, Hospitalized, and Deaths
...
topline_data = (total_cases, total_tests, total_hospitalizations, total_deaths)
...
def generate_table(dataframe, max_rows=2):
    ...
    [
        ...
        ], style={"margin-left":"auto", 'margin-right':'auto', 'width':'50%', 'background-color':'#21252C'},)
    )
    ])
...
app.layout = html.Div(children=[
    ...
    html.P("As of "+max_date, style={'font-size':'small', "text-align":"center", 'color':colors['text']}),
    ...
    html.Label(["Measures", dcc.Dropdown(id="select_metric", className = 'metric_dropdown',
                options=[
                    {"label": "Confirmed Cases", "value":'number_confirmed'},
                    {"label": "Hospitalizations", "value":'number_hospitalized'},
                    {"label": "Tests", "value":'number_tested'},
                    {"label": "% Postive Test Rate", "value":'pct_tested_positive'},
                    {"label": "Deaths", "value":'number_deaths'}],
                 value= "number_confirmed",
                 multi=False
                )],style={'color':colors['text']}),
                html.Br(),
    html.Label(["7-Day Averages", dcc.Dropdown(id="select_7_day_metric", className = 'metric_dropdown',
                options=[
                    {"label": "% Positive Test Rate", "value":'7-day_rolling_avg_pct_tested_positive'},
                    {"label": "Confirmed Cases", "value":'7-day_rolling_avg_number_confirmed'},
                    {"label": "Hospitalizations", "value":'7-day_rolling_avg_number_hospitalized'},
                    {"label": "Tests", "value":'7-day_rolling_avg_number_tested'},
                    {"label": "Deaths", "value":'7-day_rolling_avg_number_deaths'}],
                 value= "7-day_rolling_avg_pct_tested_positive",
                 multi=False
                 )],style={'color':colors['text']}),
...
@app.callback(
    ...
    Input(component_id='select_7_day_metric', component_property='value')]
)

def update_graph(metric_selected, seven_day_metric_selected):
    print(metric_selected)
    print(type(metric_selected))

    print(seven_day_metric_selected)
    print(type(seven_day_metric_selected))
    ...
    'number_tested':'Tests',
    'pct_tested_positive':'% Positive Test Rate',
    'number_deaths':'Deaths'}

    dff_dict_y = {'7-day_rolling_avg_number_confirmed':'Confirmed Cases',
    '7-day_rolling_avg_number_hospitalized':'Hospitalizations',
    '7-day_rolling_avg_number_tested':'Tests',
    '7-day_rolling_avg_pct_tested_positive':'% Positive Test Rate',
    '7-day_rolling_avg_number_deaths':'Deaths'}
    ...
    fig.add_trace(
        go.Scatter(x=dff['specimen_date_dt'], y=dff[seven_day_metric_selected], name='7-Day Average: ' +dff_dict_y[seven_day_metric_selected], mode='lines+markers', line=dict(width=1), marker=dict(size=4) ),
        secondary_y=True,
    )
